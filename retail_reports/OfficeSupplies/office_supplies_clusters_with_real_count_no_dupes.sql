...
-- set player count
update cl set players = count
from #clusters cl
inner join
(
	-- count companies per cluster
	select cl2.cluster_id, count(distinct cl2.company_id) as count
	from #clusters cl2
	group by cl2.cluster_id
) t on t.cluster_id = cl.cluster_id


-- remove any 2 player clusters that have staples in them
-- !!!jeremy requested to remove this rule
...
--	from #clusters cl
--	where cl.company_id = @SP and (cl.players = 2 or cl.players =1)
--)
...
select cl.*, a.street_number, a.street, a.municipality, a.governing_district, a.postal_area, a.longitude, a.latitude, ts.wkt_shape
from #clusters cl
inner join stores s on s.store_id = cl.store_id
inner join addresses a on a.address_id = s.address_id
inner join trade_areas t on t.store_id = s.store_id and t.threshold_id = @trade_area_threshold
inner join trade_area_shapes ts on ts.trade_area_id = t.trade_area_id
